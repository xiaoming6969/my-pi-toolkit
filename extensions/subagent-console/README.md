...
主会话 shutdown 会终止 active 与 idle reusable 子进程并释放 queued 请求、timer 和路径锁。每个 managed RPC turn 在 prompt 实际写入后有固定 30 分钟硬运行超时；排队和等待 follow-up guard 不计时。超时会发送 abort、立即拒绝同 Agent queued 请求，并继续复用现有 5 秒 settle watchdog；正常 settled 或强制 dispose 都会清 turn timer 并释放路径锁。用户取消运行中的 follow-up 同样最多等待 5 秒；若未收到 settled 就终止该进程。Overlay 的 1 秒显示刷新 timer、run 订阅和 mouse tracking 在 Esc、自动关闭、异常、dispose 或 reload 时幂等释放。运行记录默认位于系统临时目录的 `my-pi-toolkit-subagents/`，保留时间由 `~/.pi/agent/subagents.json` 的 `retainCompletedMinutes` 控制。
