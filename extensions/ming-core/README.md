...
8. `subagent-policy` — 当前会话有 `subagent` 工具时，向系统提示注入 pi-subagents 使用约定
9. `task-duration` — 在最终回复下方持久显示首次 `agent_start` 到最终 `agent_settled` 的任务耗时，不进入 LLM 上下文
10. `worktree` — `/new-worktree`、`/apply-worktree`、`/delete-worktree`，实现位于本入口的 `worktree/` 子目录
11. `startup-dashboard` — 启动面板与 Footer
12. `helps` — `/helps` 在浏览器打开 [my-pi-toolkit](https://github.com/xiaoming6969/my-pi-toolkit) 仓库页
...
探索代码与 TAPD review 使用另行安装的 [`pi-subagents`](https://github.com/nicobailon/pi-subagents)（`scout` / `reviewer`）。当前会话激活 `subagent` 时，`subagent-policy` 会向系统提示注入何时委派各角色；本入口不再注册自研子代理运行时。
