...
- Footer 使用响应式双行主布局；第一行显示 Build/Plan/Ask、项目、Git 分支、分支不匹配状态、会话标题、`provider/model`、思考强度和活跃子 Agent。`session-branch` 为 `blocked` 时 `branch mismatch` 紧跟 Git 分支名并使用 error glyph/颜色，为 `advisory` 时使用 warning 色且无 error glyph；模式配色全部来自当前 Theme，窄屏按 segment 自动换行和紧凑化。
