...
- Run the app (no path config needed): `cd /workspace && export PATH="$PWD/node_modules/.bin:$PATH" && pi --no-session`. On first launch, choose "Trust" at the project-folder prompt. The startup dashboard should list toolkit extensions `context7`, `ming-core`, `tapd`, plus companion packages `pi-lens` and `ponytail` after `npm install` / `pi install` (postinstall registers them; `session_start` backfills if scripts were skipped). `ming-core` composes the former general-purpose modules (agent-todos, chat-mode, companion-packages, cursor-models, model-manager, openai-compat-models, repo-search-subagent, startup-dashboard, subagent-console, task-duration). The toolkit is already registered in Pi user settings via `pi install .`; if it isn't loading, re-run `pi install .` from the repo root.
