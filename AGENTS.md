...
- Run the app (no path config needed): `cd /workspace && export PATH="$PWD/node_modules/.bin:$PATH" && pi --no-session`. On first launch, choose "Trust" at the project-folder prompt. The startup dashboard should list 3 extensions (`context7`, `ming-core`, `tapd`). `ming-core` composes the former general-purpose modules (agent-todos, chat-mode, model-manager, openai-compat-models, subagent, startup-dashboard, task-duration). The toolkit is already registered in Pi user settings via `pi install .`; if it isn't loading, re-run `pi install .` from the repo root.
