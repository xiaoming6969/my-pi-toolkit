...
- This repo is a Pi coding-agent plugin bundle, not a standalone server. The "application" is the Pi CLI (`@earendil-works/pi-coding-agent`, installed by `npm install` into `node_modules/.bin/pi`) loading this toolkit's extensions/skills/themes. Cloud / `pi install .` / git install have **no build step** and no ports: committed `package.json` `pi.extensions` point at TypeScript, and Pi executes those `.ts` files directly. Do not run `prepare`/`postinstall` compilation. `dist/` is produced only by `prepack` for npm tarballs.
...
- Run the app (no path config needed): `cd /workspace && export PATH="$PWD/node_modules/.bin:$PATH" && pi --no-session`. On first launch, choose "Trust" at the project-folder prompt. The startup dashboard should list 3 extensions (`context7`, `ming-core`, `tapd`). `ming-core` composes the former general-purpose modules (agent-todos, chat-mode, model-manager, openai-compat-models, subagent, startup-dashboard, task-duration, turn-diff). The toolkit is already registered in Pi user settings via `pi install .`; if it isn't loading, re-run `pi install .` from the repo root.
...
- External-API-backed extensions need their own config and are optional for loading: TAPD (`~/.pi/agent/tapd.json`), Context7 (`~/.pi/agent/context7.json` or `CONTEXT7_API_KEY`), GitLab (`gitlab.token` / `GITLAB_PERSONAL_ACCESS_TOKEN`).
- Tests/lint: 根目录执行 `npm test`（`tsx` + `node:test`）。每个模块的测试在 `extensions/<module>/test/`，仓库级检查在 `test/`。`npm run test:coverage` 输出覆盖率。单文件：`npm test -- extensions/tapd/test/parser.test.ts`。不要假设存在根 `tsconfig` 或 `npm run lint`。约定见 [`docs/testing.md`](docs/testing.md)。
