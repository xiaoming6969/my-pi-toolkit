...
- `GET /bugs/get_fields_info` 返回 Bug 标准字段及候选值；合入版本、根因大类、开发人员、修复方案说明、根因分析（RCA）、影响范围必须按中文 `label` 动态定位（合入版本当前项目字段名为 `version_fix`）。根因大类为 `cascade_radio` 时，子 Agent 先从「大类 / 子项」候选中选一项，无法确定再在 MR 确认前让用户先选大类再选子类；项目有该字段时必选，写入值为 `大类/子项`。三个 textarea 写入纯文本。开发人员写入当前 Token 用户 `nick;`。项目没有对应字段时跳过该项且不阻断流转。
