...
- `GET /bugs/get_fields_info` 返回 Bug 标准字段及候选值；合入版本、根因大类、开发人员字段必须按中文 `label` 动态定位（合入版本当前项目字段名为 `version_fix`）。根因大类为 `cascade_radio` 时，主 Agent（优先 `reviewer`）先从「大类 / 子项」候选中选一项，无法确定再让用户先选大类再选子类；写入值为 `大类/子项`。开发人员写入当前 Token 用户 `nick;`。
